...
export const isYear = (str: string) => {
  ...
  const [start, end] = str.split('-').map(Number) as [number, number];
  if (start <= end && 1920 <= start && end <= new Date().getFullYear())
    return true;
  return false;
};

export const isAgeRating = (str: string) => {
  if (!/\d\d?-\d\d?/.test(str)) return false;
  const [start, end] = str.split('-').map(Number) as [number, number];
  if (start <= end && 0 <= start && end <= 18) return true;
  return false;
};
